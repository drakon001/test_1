...
from server.models.schemas import SubmissionData, SubmitRequest
...
class MongoStorage():
    ...
    def filter(self, request: SubmitRequest):
        ...
        try:
            ...
            # Если результатов меньше 2, добавляем сгенерированные данные, на всякий случай 
            if len(results) < 2:
                num_items = max(2, 5 - len(results))
